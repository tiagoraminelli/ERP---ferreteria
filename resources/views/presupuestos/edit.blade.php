        ...
        <div class="flex justify-between items-center">
            <div>
                ...
                <p class="text-sm text-white-500 dark:text-gray-400">
                    Modificar presupuesto #{{ $presupuesto->id }}
                </p>
            </div>
...
'{{ $producto->precio }}'
                    ...
                    <div>
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px">
                                    ...
                                    <select name="cliente_id" id="select2Clientes" style="width:100%">
                                        ...
                                        @foreach ($clientes as $cliente)
                                            <option value="{{ $cliente->id }}"
                                                {{ old('cliente_id', $presupuesto->cliente_id) == $cliente->id ? 'selected' : '' }}>
                                                {{ $cliente->nombre }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div>
                                    ...
                                    <select name="estado" id="estado"
                                        ...
                                        <option value="borrador" {{ old('estado', $presupuesto->estado) == 'borrador' ? 'selected' : '' }}>
                                            Borrador</option>
                                        <option value="enviado" {{ old('estado', $presupuesto->estado) == 'enviado' ? 'selected' : '' }}>
                                            Enviado</option>
                                ...
                                <div>
                                    ...
                                    <input type="date" name="fecha" id="fecha"
                                        value="{{ old('fecha', $presupuesto->fecha ? \Carbon\Carbon::parse($presupuesto->fecha)->format('Y-m-d') : date('Y-m-d')) }}"
                                ...
                                <div style="grid-column:span 2">
                                    ...
                                    <textarea name="observaciones" rows="2"
                                        style="width:100%;padding:8px;border:1px solid #ddd;border-radius:4px;box-sizing:border-box">{{ old('observaciones', $presupuesto->notas) }}</textarea>
                                </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td style="text-align:center">
                                                            <input type="number" step="0.01"
                                                                ...
                                                                value="{{ old("productos.$index.precio", $detalle->precio) }}"
                                                                oninput="calcular()"
                                                        ...
                                                        <td style="text-align:center">
                                                            <input type="number" step="0.01"
                                                                ...
                                                                value="{{ old("productos.$index.descuento_porcentaje", $detalle->descuento_porcentaje) }}"
                                                                oninput="calcular()"
                                                        ...
                                                        <td style="text-align:center">
                                                            <input type="number" step="0.01"
                                                                ...
                                                                value="{{ old("productos.$index.descuento_monto", $detalle->descuento_monto) }}"
                                                                oninput="calcular()"
                                                                style="width:60px;padding:5px;border:1px solid #ddd;border-radius:4px">
                                                        </td>
                                                        <td class="subtotal"
                        ...
                        <div
                            ...
                            <div style="text-align:right">
                                ...
                                <div id="totalPresupuesto" style="font-size:24px;font-weight:bold;color:#059669">
                                    $ {{ number_format($presupuesto->total, 2) }}
                                </div>
                            </div>

                            <button type="submit"
                                style="width:100%;padding:12px;background:#1f2937;color:white;border:none;border-radius:6px;font-weight:600;cursor:pointer;transition:background 0.2s;margin-top:20px"
    ...
    <script>
        let indexProducto = {{ $presupuesto->detalles->count() }}
        ...
        $(document).ready(function() {
            ...
            // Inicializar contador y cálculos si hay productos
            actualizarContador();
            if (document.querySelectorAll("#tablaProductos tr").length > 0) {
                calcular();
            }
        });
    </script>
